...
export function NavMobile() {
  ...
  return (
    <>
      {/* Bottom bar */}
      <div className="md:hidden fixed bottom-0 inset-x-0 w-full z-40 glass border-t border-border/50 will-change-transform">
        <div className="flex justify-between items-center px-5 py-3 pr-16">
          <button
      ...
      {shouldRender && (
        <>
          ...
          <div
            ...
            className={`md:hidden fixed inset-0 z-50 bg-black/60 presence-fade ${isVisible ? "presence-visible" : ""}`}
          />
          {/* Sheet */}
          <div
            className={`md:hidden fixed bottom-0 left-0 right-0 z-50 glass rounded-t-2xl border-t border-border/50 presence-slide-up ${isVisible ? "presence-visible" : ""}`}
          >
            <div className="p-5">
              {/* Handle */}
