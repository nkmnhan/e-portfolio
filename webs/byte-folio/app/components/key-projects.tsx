...
function KeyProjectCard({ project, index }: { project: KeyProject; index: number }) {
  ...
  return (
    <article
      ref={ref}
      style={{
        "--_glow-color": `var(${accentGlowVar[project.accentColor]})`,
        transitionDelay: `${index * 0.12}s`,
      } as React.CSSProperties}
      className={`group relative glass rounded-xl border-t-2 ${accentBorder[project.accentColor]} view-hidden ${isInView ? "view-visible" : ""}`}
    >
    ...
    <SectionWrapper id="key-projects">
      ...
      <div className="grid grid-cols-1 gap-6">
        {keyProjectsData.map((project, index) => (
          <KeyProjectCard key={project.id} project={project} index={index} />
        ))}
      </div>
    </SectionWrapper>
  );
